...
with open("example.com", 'rb') as data:
    data = data.read()
disasm = unazed_disasm.Disassembler(data, unazed_disasm.OPCODE_MAP, org=0x100)
offs = 0
for instr, *operands in disasm.iterate_instructions():
    ...
    print(f"+{format(offs, 'x'):10s} {bytes_:20s} {instr.op_ident % operands[0] if operands else instr.op_ident:20s} {''.join(ascii_):4s} {instr._note}")
